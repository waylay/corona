...
function format ( d ) {
    return '<div class="details"><form class="details-form">' +
        '<div class="col-sm-4">' +
            ...
            '<strong>Submission Date:</strong> ' + d.created_at + '<br>' +
            '<strong>IMEI:</strong> ' + d.imei + '<br>' +
        ...
        '<div class="col-sm-4">' +
            ...
            d.phone + '<br>' +
            '<a class="text-info" href="mailto:'+ d.email +'">' + d.email + '</a><br>' +
        '</div>' +
        '<div class="col-sm-4">' +
            '<p><a target="_blank" class="text-info" href="' + d.receipt + '"> View Receipt </a></p>' +
            '<label class="form-check-label"><input type="checkbox" id="emailed" name="emailed" ' + ( d.emailed ? ' checked ' : '' ) + '">Sent email confirmation</label>' +
            '<label class="form-check-label"><input type="checkbox" id="validated" name="validated" ' + ( d.validated ? ' checked ' : '' ) + '">IMEI Code validated</label>' +
            '<label class="form-check-label"><input type="checkbox" id="shipped" name="shipped" ' + ( d.shipped ? ' checked ' : '' ) + '">Product shipped</label>' +
        '</div>' +
        '<div class="col-sm-8">' +
            '<br><label class="form-check-label"><strong>Admin notes:</strong></label>' +
            show_notes(d.notes) +
            '<textarea name="note" class="form-control" placeholder="Add note"></textarea>' +
        '</div>' +
        '<div class="col-sm-4">' +
            '<button class="btn btn-success btn-save btn-md text-uppercase save" type="submit">SAVE</button>' +
...
jQuery(document).ready(function($) {
    ...
    var table = $('#entries').DataTable({
        ...
        columns: [
            ...
            {'data': 'lastname', 'responsivePriority': 3},
            {'data': 'phone'},
            {'data': 'email'},
            {'data': 'address'},
            {'data': 'address2'},
            ...
            {'data': 'emailed',
                ...
                render: function ( data, type, row ) {
                    return '<span class="label label-' + (data ? 'success' : 'danger') +' ">'+
                                (data ? 'Y' : 'N')
                          +'</span>';
            ...
            {'data': 'notes',
                ...
                render: function ( data, type, row ) {
                    return '<span class="label label-' + ( data.length ? 'success' : 'danger') +' ">'+
                        ( data.length ? 'Y' : 'N')
                        +'</span>';
    ...
    $( '#entries tbody' ).on( "submit", ".details-form", function( event ) {
        ...
        $.ajax({
            ...
            success: function( response ) {
                ...
                $('.details .btn-save', table.row(idx).child()).toggleClass('btn-success')
                    .text('Updating..')
                    .prop('disabled', true);
                setTimeout(function() {
                    $('.details .btn-save', table.row(idx).child()).toggleClass('btn-success')
